...
var comlink = require("@sanity/comlink"), react = require("react");
...
function useDocumentsInUse(documentsInUse, projectId, dataset) {
  const [comlink$1, setComlink] = react.useState(null), [connected, setConnected] = react.useState(!1);
  react.useEffect(() => {
    if (window.self === window.top && !window.opener)
      return;
    const comlink2 = comlink.createNode(
      {
        name: "preview-kit",
    ...
    return () => {
      stop(), setConnected(!1), setComlink(null), clearTimeout(timeout);
    };
  }, [dataset, projectId]);
  const changedKeys = JSON.stringify(Array.from(documentsInUse.keys()));
  react.useEffect(() => {
    changedKeys !== "[]" && comlink$1 && connected && comlink$1.post("preview-kit/documents", {
      projectId,
      dataset,
      perspective: "previewDrafts",
      documents: Array.from(documentsInUse.values())
    });
  }, [changedKeys, comlink$1, connected, dataset, documentsInUse, projectId]);
}
...
exports.useDocumentsInUse = useDocumentsInUse;
