...
const channelsToComlinkMap = {
  ...
  return message.domain === comlink.DOMAIN && (message.domain = "sanity/channels"), message.to === "visual-editing" && (message.to = "overlays"), message.from === "visual-editing" && (message.from = "overlays"), message.type = comlinkToChannelsMap[message.type] ?? message.type, message.type === "channel/response" && message.responseTo && !message.data && (message.data = {
...
const PostMessageFeatures = (props) => {
  ...
  let t0, t1;
  return $[0] !== comlink2 ? (t0 = () => comlink2.on("visual-editing/features", _temp$8), t1 = [comlink2], $[0] = comlink2, $[1] = t0, $[2] = t1) : (t0 = $[1], t1 = $[2]), react.useEffect(t0, t1), null;
};
...
function validateWidths(panels, widthsToValidate, containerWidth) {
  ...
  if (remainingWidth.toFixed(3) !== "0.000")
    for (let index2 = 0; index2 < panels.length; index2++) {
      ...
      const width = Math.min(maxWidth ?? 100, Math.max(minWidth, widths[index2] + remainingWidth));
...
function DocumentListPane(props) {
  ...
  if ($[0] !== mainDocumentState?.document?._id || $[1] !== refs) {
    let t22;
    $[3] !== mainDocumentState?.document?._id ? (t22 = (r) => sanity.getPublishedId(r._id) !== mainDocumentState?.document?._id, $[3] = mainDocumentState?.document?._id, $[4] = t22) : t22 = $[4], t1 = refs.filter(t22).map(_temp$5), $[0] = mainDocumentState?.document?._id, $[1] = refs, $[2] = t1;
  } else
    t1 = $[2];
  t0 = t1;
  const ids = t0;
...
const PERSPECTIVE_TITLE_KEY = {
  ...
  let t0;
  $[0] !== props ? (t0 = props.options?.component ? react.createElement(props.options.component, {
    ...props,
    renderDefault
...
const MotionFlex = framerMotion.motion.create(ui.Flex), Preview = react.memo(react.forwardRef(function(props, forwardedRef) {
  const {
    ...
    devMode
  } = index.usePresentationTool(), prefersReducedMotion = ui.usePrefersReducedMotion(), ref = react.useRef(null), PreviewHeader2 = usePresentationPreviewHeader({
    ...props,
    iframeRef: ref,
    options: header
  });
...
function PresentationTool(props) {
  const {
    ...
    canUseSharedPreviewAccess,
    tool
  } = props, components = tool.options?.components, _previewUrl = tool.options?.previewUrl, name = tool.name || index.DEFAULT_TOOL_NAME, {
    unstable_navigator,
    unstable_header
  } = components || {}, {
...
function PresentationToolGrantsCheck(props) {
  ...
  let t7;
  return $[11] !== props || $[12] !== t4 || $[13] !== t5 || $[14] !== t6 ? (t7 = /* @__PURE__ */ jsxRuntime.jsx(PresentationTool, { ...props, canCreateUrlPreviewSecrets: t4, canToggleSharePreviewAccess: t5, canUseSharedPreviewAccess: t6 }), $[11] = props, $[12] = t4, $[13] = t5, $[14] = t6, $[15] = t7) : t7 = $[15], t7;
}
